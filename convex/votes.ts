...
export const castVote = mutation({
  ...
  handler: async (ctx, args) => {
    // 1. Verify authenticated and first login complete
    ...
    // 3. Check for duplicate vote in this category
    ...
    await ctx.db.patch(args.candidateId, { count: candidate.count + 1 });
...
export const myVotingProgress = query({
  ...
  handler: async (ctx, args) => {
    const user = await getUser(ctx);
    // Use partial index scan on studentId, then filter by electionId in JS
    const votes = await ctx.db
      .query("voted_log")
      .withIndex("by_voter_category", (q) => q.eq("studentId", user._id))
      .take(100);
    return votes
      .filter((row) => row.electionId === args.electionId)

...
export default function ElectionDetailPage() {
  ...
  if (!election) {
    return (
      <div className="py-12 text-center max-w-sm mx-auto px-4 space-y-4">
        ...
        <button onClick={() => router.push('/admin/elections')} className="px-4 py-2 bg-secondary text-secondary-foreground rounded-lg text-sm font-medium">
          Return to List
        </button>
      </div>
    );
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      await addCategory({ electionId, name: newCategoryName, description: newCategoryDesc || undefined });
      setNewCategoryName('');
      setNewCategoryDesc('');
  ...
  const handleAddCandidate = async (e: React.FormEvent, categoryId: string) => {
    e.preventDefault();
  ...
  if (election.categories.length === 0) {
    validationErrors.push('Must configure at least 1 position category.');
  } else {
    election.categories.forEach((cat) => {
      if (cat.candidates.length === 0)
        validationErrors.push(`"${cat.name}" has no registered candidates.`);
    });
  }
    ...
    <div className="py-4 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto w-full space-y-6 animate-fade-in font-sans">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-border">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/admin/elections')} className="p-1.5 hover:bg-muted rounded-lg text-muted-foreground hover:text-foreground transition-all">
            <HugeiconsIcon icon={ArrowLeft01Icon} className="h-4.5 w-4.5" />
          </button>
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <StatusBadge status={election.status} />
              <span className="text-[10px] font-mono text-muted-foreground/60">ID: {election._id}</span>
            </div>
            <h3 className="font-sans font-bold text-lg text-foreground leading-tight">{election.title}</h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {election.status === 'draft' && (
            <button
              ...
              disabled={!canLock}
              className={`px-4 py-2 text-xs font-semibold rounded-xl flex items-center gap-1.5 transition-all shadow-sm ${canLock ? 'bg-primary text-primary-foreground hover:bg-primary/90 cursor-pointer' : 'bg-muted text-muted-foreground border border-border cursor-not-allowed'}`}
            >
          ...
          {['active', 'closed', 'published'].includes(election.status) && (
            <button
              onClick={() => router.push(`/admin/elections/${election._id}/live`)}
      ...
      {isEditable && validationErrors.length > 0 && (
        <div className="bg-[#fef3c7] border border-[#fcd34d] p-4 rounded-xl flex gap-3 text-xs leading-relaxed">
          ...
          <div className="space-y-1">
            <p className="font-semibold text-[#92400e]">Setup incomplete ({validationErrors.length} issue{validationErrors.length > 1 ? 's' : ''})</p>
      ...
      <div className="flex border-b border-border gap-1 select-none">
        {(['categories', 'candidates'] as const).map((tab) => (
          <button
            ...
            className={`px-4 py-2.5 text-xs font-semibold border-b-2 transition-all capitalize ${activeTab === tab ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
            {tab === 'categories' ? `Categories (${election.categories.length})` : 'Candidate Registry'}
          </button>
        ))}
      </div>
            ...
            <div className="p-5 border border-border bg-card rounded-xl shadow-sm animate-fade-in space-y-4">
              ...
              <form onSubmit={handleCreateCategory} className="space-y-3">
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <button type="button" onClick={() => setShowAddCategoryForm(false)} className="px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted border border-border rounded-lg">Cancel</button>
                  <button type="submit" className="px-4 py-1.5 text-xs font-semibold text-primary-foreground bg-primary hover:bg-primary/90 rounded-lg shadow-sm">Add Category</button>
                </div>
              </form>
            </div>
          )}
          ...
          {election.categories.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {election.categories.map((cat) => (
                <div key={cat._id} className="border border-border bg-card p-5 rounded-xl flex flex-col gap-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="space-y-0.5 min-w-0">
                      <h4 className="font-sans font-bold text-sm text-foreground leading-tight truncate">{cat.name}</h4>
                      <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{cat.description || 'No description.'}</p>
                    </div>
                    {isEditable && (
                      <button
                        onClick={() => removeCategory({ electionId, categoryId: cat._id })}
                  ...
                  <div className="pt-2 border-t border-border text-[10px] font-mono text-muted-foreground">
                    Candidates: <strong className="text-foreground">{cat.candidates.length}</strong>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === 'candidates' && (
        <div className="space-y-5">
          {election.categories.length === 0 ? (
            ...
            election.categories.map((cat) => {
              const isAddingToThisCat = expandedCategoryId === cat._id;
              return (
                <div key={cat._id} className="border border-border bg-card rounded-2xl overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                    <div>
                      <h4 className="font-sans font-bold text-sm text-foreground">{cat.name}</h4>
                      <p className="text-[11px] text-muted-foreground mt-0.5">{cat.candidates.length} candidate{cat.candidates.length !== 1 ? 's' : ''} registered</p>
                    </div>
                    {isEditable && !isAddingToThisCat && (
                      <button
                        onClick={() => setExpandedCategoryId(cat._id)}
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    {cat.candidates.length === 0 ? (
                      <div className="text-center py-8 text-xs text-muted-foreground border border-dashed border-border rounded-xl">No candidates registered yet.</div>
                    ) : (
                      <div className="space-y-2">
                        {cat.candidates.map((cand) => (
                          <div key={cand._id} className="border border-border rounded-xl p-4 flex items-start justify-between gap-4 hover:bg-muted/30 transition-colors">
                            <div className="space-y-1 min-w-0">
                              <h5 className="font-sans font-semibold text-sm text-foreground leading-tight">{cand.userName}</h5>
                              <span className="block font-mono text-[10px] text-muted-foreground uppercase">{cand.userId}</span>
                              {cand.bio && <p className="text-xs text-muted-foreground leading-relaxed mt-1 pr-2">{cand.bio}</p>}
                            </div>
                            {isEditable && (
                              <button
                                onClick={() => removeCandidate({ electionId, candidateId: cand._id })}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                  This will immediately stop all voting for <strong className="text-foreground">{election.title}</strong>. This cannot be undone.
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setShowCloseConfirm(false)} className="flex-1 px-4 py-2.5 text-sm font-medium text-foreground border border-border rounded-xl hover:bg-muted transition-all">Cancel</button>
              <button onClick={handleEarlyClose} className="flex-1 px-4 py-2.5 text-sm font-semibold text-destructive-foreground bg-destructive hover:bg-destructive/90 rounded-xl transition-all">Close Election</button>
            </div>
          </div>
        </div>,
        document.body

    ...
    <div className="space-y-6 font-sans py-4 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full animate-fade-in select-none">
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-mono text-muted-foreground animate-pulse">● Live counts stream</span>
        </div>

        {resultCategories.length === 0 ? (
          <div className="bg-card border border-dashed border-border rounded-2xl p-8 text-center">
            <p className="text-xs text-muted-foreground">
              {results === undefined ? 'Loading tallies…' : 'No tally data available.'}
            </p>
          </div>
        ) : (
          resultCategories.map((cat) => {
            const sorted = [...cat.candidates].sort((a, b) => b.count - a.count);
            const totalVotes = cat.candidates.reduce((a, c) => a + c.count, 0);
              ...
              <div key={cat._id} className="bg-card border border-border p-6 rounded-2xl space-y-4">
                ...
                <div className="space-y-4 select-text">
                  {sorted.map((cand, idx) => {
                    ...
                    const isLeader = idx === 0 && cand.count > 0 && currentElection.status !== 'active';
